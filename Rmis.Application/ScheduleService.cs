...
namespace Rmis.Application
{
    internal class ScheduleService : IScheduleService
    {
        ...
        public IEnumerable<ScheduleVm> GetSchedulesByRouteNumber(int routeNumber, DateTimeOffset date)
        {

            return _context.ScheduleRepository.GetActualAllByRouteNumber(routeNumber)
                .Select(s => ScheduleVm.CreateFrom(s))
                .ToList()
                .Where(s => s.ArrivalDate == date);
        }
    }
}